            ...
            try {
                ...
                if( u != null ) {
                    ...
                    if( newPassword != null && !newPassword.isEmpty() && newPassword.equals(confirmPassword) && !Utility.hashString(newPassword).equals(u.getPassword()) ){
                        ...
                        try {
                            String message = "Your password has been changed.\n\nIf you did not initiate this request contact security@HeraPass.com immediately to freeze your account from unauthorized access.\n";
                            MessageService.send("Forgot Password Request from HeraPass.com", message, null, userid, NotificationType.EMAIL);
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
